subspaces=[10 20 50 100 200 500];
...
%%Randomized rather than ordered
    ...
    yValSub=yValSub*2-1;
        ...
        yValSub=2*yValSub-1;
    tp=sum((yValSub==1)&(yVal==1));
    fp=sum((yValSub==1)&(yVal==-1));
    fn=sum((yValSub==-1)&(yVal==1));
    tn=sum((yValSub==-1)&(yVal==-1));
    accuracy=(tp*classImba+tn) /((tp+fn)*classImba+tn+fp);
    valAcc(kNo,i)=accuracy;
    
    lSub=[ones(nTest,1) tData(:,subspace)]*wSub';
    ySub=lSub(:,1)<lSub(:,2);
    ySub=ySub*2-1;
    tp=sum((ySub==1)&(yTest==1));
    fp=sum((ySub==1)&(yTest==-1));
    fn=sum((ySub==-1)&(yTest==1));
    tn=sum((ySub==-1)&(yTest==-1));
    accuracy=(tp*classImba+tn) /((tp+fn)*classImba+tn+fp);
...
%   
    tp=sum((yEns==1)&(yTest==1));
    fp=sum((yEns==1)&(yTest==-1));
    fn=sum((yEns==-1)&(yTest==1));
    tn=sum((yEns==-1)&(yTest==-1));
    accuracy=(tp*classImba+tn) /((tp+fn)*classImba+tn+fp);
...
%    valAccW=exp(reshape(valAcc(sNo,thetaNo,kNo,1:i),1,[])-1); 
    ...
    tp=sum((yEns1==1)&(yTest==1));
    fp=sum((yEns1==1)&(yTest==-1));
    fn=sum((yEns1==-1)&(yTest==1));
    tn=sum((yEns1==-1)&(yTest==-1));
    accuracy=(tp*classImba+tn) /((tp+fn)*classImba+tn+fp);
...
%    accuracy=sum(yEns1==yTest)/nTest;
    ...
    tp=sum((yEns2==1)&(yTest==1));
    fp=sum((yEns2==1)&(yTest==-1));
    fn=sum((yEns2==-1)&(yTest==1));
    tn=sum((yEns2==-1)&(yTest==-1));
    accuracy=(tp*classImba+tn) /((tp+fn)*classImba+tn+fp);
...
%    accuracy=sum(yEns2==yTest)/nTest;
    ...
    tp=sum((yEns3==1)&(yTest==1));
    fp=sum((yEns3==1)&(yTest==-1));
    fn=sum((yEns3==-1)&(yTest==1));
    tn=sum((yEns3==-1)&(yTest==-1));
    accuracy=(tp*classImba+tn) /((tp+fn)*classImba+tn+fp);
    
%    accuracy=sum(yEns3==yTest)/nTest;
    ...
    tp=sum((yEns4==1)&(yTest==1));
    fp=sum((yEns4==1)&(yTest==-1));
    fn=sum((yEns4==-1)&(yTest==1));
    tn=sum((yEns4==-1)&(yTest==-1));
    accuracy=(tp*classImba+tn) /((tp+fn)*classImba+tn+fp);   
...
%    accuracy=sum(yEns4==yTest)/nTest;
    ...
    tp=sum((yEns5==1)&(yTest==1));
    fp=sum((yEns5==1)&(yTest==-1));
    fn=sum((yEns5==-1)&(yTest==1));
    tn=sum((yEns5==-1)&(yTest==-1));
    accuracy=(tp*classImba+tn) /((tp+fn)*classImba+tn+fp);   
...
%   betaDist(sNo,thetaNo,kNo,2)=params(2);
  ...
  ensCor=corr(yEnsRec(:,:),yEnsRec(:,:));
  r(kNo)=(sum(sum(ensCor))-ensSize)/ensSize/(ensSize-1)/2;  
  psi(kNo)=r(kNo)/(1-r(kNo));
%  ensCor=corr(weightsSub(:,:)',weightsSub(:,:)');
%  r2(kNo)=(sum(sum(ensCor))-ensSize)/ensSize/(ensSize-1)/2;
%  psi2(kNo)=r2(kNo)/(1-r2(kNo));
  

  temp=reshape(yEnsScore(:,1,:),nTest,[]);
  ensCor=corr(temp,temp);
  ensCor(isnan(ensCor))=1;
  r3(kNo)=(sum(sum(ensCor))-ensSize)/ensSize/(ensSize-1)/2;

  psi3(kNo)=r3(kNo)/(1-r3(kNo));
  

    pk(kNo)=mean(accSub(kNo,:));
...
%   end
   ...
   if (psi3(kNo)<(pk(kNo)-1)/(ensSize-1))
       psi3(kNo)=(pk(kNo)-1)/ensSize
   end
end
...
close gcf;
sTitle=sprintf('Ensemble Accuracy vs Ensemble Size for Dataset: DOROTHEA, Training size=%d, Test Size=%d, d=%d',nTrain, nTest,d);
...
i=0;
    ...
    drawnow;
    fileName=sprintf('weightedVote_dorothea_scaledAcc.fig');
    savefig(fileName);
    fileName=sprintf('weightedVote_dorothea_scaledAcc.eps');
    saveas(gcf,fileName,'epsc');


    close gcf;
sTitle=sprintf('Ensemble Accuracy vs Ensemble Size for DOROTHEA');
...
%    psi2=sqrt(r2(sNo,thetaNo,kNo))/(1-sqrt(r2(sNo,thetaNo,kNo)));   
 ...
 %       myPlots(6)=plot(rMLE,'-.');
    ...
    legend(myPlots,legendCell,'Location',[0.4 0.045 0.3 0.04]);
    drawnow;
    fileName=sprintf('polyaModel_dorothea.fig');
    savefig(fileName);
    fileName=sprintf('polyaModel_dorothea.eps');
    saveas(gcf,fileName,'epsc');
