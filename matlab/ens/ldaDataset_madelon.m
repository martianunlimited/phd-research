...
accBase=0;
r2=zeros(runCount,kCount,1);
r=zeros(runCount,kCount,1);
r3=zeros(runCount,kCount,1);
psi3=zeros(runCount,kCount,1);
psi=zeros(runCount,kCount,1);
psi2=zeros(runCount,kCount,1);
divesMeas=zeros(kCount,1);
betaDist=zeros(kCount,2);
pk=zeros(runCount,kCount,1);
U4=0;
UUT2=0;
...
close gcf;
sTitle=sprintf('Ensemble Accuracy vs Ensemble Size for Dataset: MADELON, Training size=%d, Test Size=%d, d=%d',nTrain, nTest,d);
...
i=0;
    ...
    drawnow;
    fileName=sprintf('weightedVote_madelon.fig');
    savefig(fileName);
    fileName=sprintf('weightedVote_madelon.eps');
    saveas(gcf,fileName,'epsc');


    
    close gcf;
sTitle=sprintf('Ensemble Accuracy vs Ensemble Size for MADELON');
...
%    psi2=sqrt(r2(sNo,thetaNo,kNo))/(1-sqrt(r2(sNo,thetaNo,kNo)));   
 ...
 %       myPlots(6)=plot(rMLE,'-.');
    ...
    legend(myPlots,legendCell,'Location',[0.4 0.045 0.3 0.04]);
    drawnow;
    fileName=sprintf('polyaModel_madelon.fig');
    savefig(fileName);
    fileName=sprintf('polyaModel_madelon.eps');
    saveas(gcf,fileName,'epsc');
