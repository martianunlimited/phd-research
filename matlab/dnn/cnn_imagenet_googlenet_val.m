...
%for i=1:5 
    %sprintf('%s (%d), score %.3f',...net.meta.classes.description{sortingIndices(i)}, sortingIndices(i),sortedX(i)) %end
        ...
        load('googlenet/dirCount.mat','dirCount');
        dirCount=dirCount+1;
        dirName=sprintf('googlenet/run_%d',dirCount);
        mkdir(dirName);
...
for fileNo=1:50000
    try
        ...
        maxXY=min(subspace^2,ImSizes(1)*ImSizes(2));
...
%im2(:,:,3) = im(sort(randperm(ImSizes(1),maxX)),sort(randperm(ImSizes(2),maxY)),3);        
         im_ = single(im2) ; % note: 255 range
%        imflat=reshape(im,1,ImSizes(1)*ImSizes(2),3);
%        im_=single(zeros(size(imflat)));
%        n=sort(randperm(ImSizes(1)*ImSizes(2),maxXY));
%        im_(1,n,:)=imflat(1,n,:);
%        im_=reshape(im_,ImSizes(1),ImSizes(2),3);
...
%toc;
        ...
        resultFile=sprintf('googlenet/run_%d/result_%d.mat',dirCount,subspace);
        save(resultFile,'results');
        toc
        end
save('googlenet/dirCount.mat','dirCount');   
