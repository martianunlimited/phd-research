...
modelPath = 'data/models/imagenet-resnet-152-dag.mat' ;
...
%for i=1:5 
    %sprintf('%s (%d), score %.3f',...net.meta.classes.description{sortingIndices(i)}, sortingIndices(i),sortedX(i)) %end
        ...
        load('resnet152/dirCount.mat','dirCount');
        dirCount=dirCount+1;
        dirName=sprintf('resnet152/run_%d',dirCount);
        mkdir(dirName);
        %subspaces=[200 150 100];
...
%toc;
        ...
        resultFile=sprintf('resnet152/run_%d/result_%d.mat',dirCount,subspace);
        save(resultFile,'results');
        toc
        end
save('resnet152/dirCount.mat','dirCount');   