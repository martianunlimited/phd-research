...
tensorboard=TensorBoard(log_dir='./exp8', histogram_freq=1, write_graph=True, write_images=False)
...
input5=Input(shape=(1000,))
input6=Input(shape=(1000,))
input7=Input(shape=(1000,))
input8=Input(shape=(1000,))
concatL=Concatenate()([input1,input2,input3,input4,input5,input6,input7,input8])
...
model=Model([input1,input2,input3, input4,input5,input6,input7,input8],out)
...
history=model.fit_generator(myGeneratorTrain(), steps_per_epoch = 400 , epochs = 100, verbose=2, validation_data=([sumX1val/ensSize, sumX2val/ensSize, sumX3val/ensSize, sumX4val/ensSize,maxX1val,maxX2val,maxX3val,maxX4val],Y_val),callbacks=[tensorboard, earlystop] )
model.save('exp8_ens_3_subs_350.h5');
np.savez('nnIndices.npz',trainIdx=trainIdx,testIdx=testIdx);
