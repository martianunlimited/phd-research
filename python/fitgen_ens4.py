...
M=scio.loadmat('best3_train_450.mat')
X=M["best3"]
...
def myGeneratorVal():
    M=scio.loadmat('best3_val_450.mat')
    X_val=M["best3"]
    X_val=X[testIdx,:]
    M=scio.loadmat('solnArry.mat')
    Y_val=M["solnArry"]
...
layer1=Dense(nb_hidden,kernel_regularizer=regularizers.l1(0.00001),bias_initializer='zeros', activation='relu')(concatL)
#layer1=Dropout(0.5)(layer1)
out=Dense(nb_classes,kernel_regularizer=regularizers.l1(0.00001),bias_initializer='zeros', kernel_initializer='identity',activation='softmax')(layer1)
...
history=model.fit_generator(myGeneratorTrain(), steps_per_epoch = 400 , epochs = 50, verbose=2, validation_data=myGeneratorVal(), validation_steps=100 )

model.evaluate([X_test[:,0:1000],X_test[:,1000:2000],X_test[:,2000:3000]],np_utils.to_categorical(Y_test-1,1000))
